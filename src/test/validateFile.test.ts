...
suite('File-level validate', () => {
  suite('filterDiagnosticsForFile', () => {
    test('returns only diagnostics matching the active file basename', () => {
      const output = [
        'reviewer.xcf:3:1: unknown field "invalid-field"',
        'my-skill.xcf:5:1: missing required field "name"',
        'reviewer.xcf:7:10: value must be a string',
      ].join('\n');

      const filtered = filterDiagnosticsForFile(output, '/workspace/xcf/agents/reviewer.xcf');
    ...
    test('returns empty array when no diagnostics match the file', () => {
      const output = 'other.xcf:1:1: some error';
      const filtered = filterDiagnosticsForFile(output, '/workspace/xcf/agents/reviewer.xcf');
      assert.strictEqual(filtered.length, 0);
    });

    test('returns empty array for empty output', () => {
      const filtered = filterDiagnosticsForFile('', '/workspace/xcf/agents/reviewer.xcf');
      assert.strictEqual(filtered.length, 0);
    });

    test('returns fallback diagnostic for validation-failed without line info', () => {
      const output = 'Validation failed: reviewer.xcf has errors';
      const filtered = filterDiagnosticsForFile(output, '/workspace/xcf/agents/reviewer.xcf');
    ...
    test('handles path with special regex characters', () => {
      const output = 'my+skill.xcf:2:1: bad field';
      const filtered = filterDiagnosticsForFile(output, '/workspace/my+skill.xcf');
      assert.strictEqual(filtered.length, 1);
    });
  });
});
